...
void D3Empty(string Lattice[L][L][L], int x, int y, int z, int direction, int& chosen,
	...
	*/
	...
	for (int run = 1; run <= 1; run++)
	{
		...
		while (a < spots && b < spots && c < spots && d < spots && a+c < spots && b+d < spots)
		{
			...
			else if (Lattice[x][y][z] == "0")
			{
				int direction = genrand_int32() % 2;
				//choosing direction out of 2 (1 left, 0 right)
				D3Empty(Lattice, x, y, z, direction, a,
					"a", BirthRateA, SwitchRate);
				D3Empty(Lattice, x, y, z, direction, c,
					"c", BirthRateC, SwitchRate);
				D3Empty(Lattice, x, y, z, direction, b,
					"b", BirthRateB, SwitchRate);
				D3Empty(Lattice, x, y, z, direction, d,
					"d", BirthRateD, SwitchRate);
			}
	...
	double BirthRatepredator, double DeathRatepredator, double BirthRateprey, double DeathRateprey, double SwitchRate)
{
	...
	else if (direction == 2)//chosen direction left
	{
		...
		else if (Lattice[(x - 1 + 2 * L) % (L)][y][z] == "0")
		{
			if (Event == 0)
			{
				Lattice[x][y][z] = "0";
	...
	else if (direction == 3)//chosen direction down 
	{
		...
		else if (Lattice[x][(y - 1 + 2 * L) % L][z] == "0")
		{
			if (Event == 0)
			{
				Lattice[x][y][z] = "0";
	...
	else if (direction == 5)//  negative z
	{
		...
		else if (Lattice[x][y][(z - 1 + 2 * L) % (L)] == "0")
		{
			if (Event == 0)
			{
				Lattice[x][y][z] = "0";
	...
	string chosenstring, double BirthRatechosen, double SwitchRate)
{
	...
	{
		if (Lattice[(x + 1) % L][y][z] == chosenstring)
		{
			if (Event == 0)
			{
				if (RandSwitch <= SwitchRate)
				{
					Lattice[x][y][z] = chosenstring;
					Lattice[(x + 1) % L][y][z] = "0";
				}
			}
			else if (Event == 1)
	...
	if (direction == 1)//up
	{
		if (Lattice[x][(y + 1) % L][z] == chosenstring)
		{
			if (Event == 0)
			{
				if (RandSwitch <= SwitchRate)
	...
	{
		if (Lattice[(x - 1 + 2 * L) % (L)][y][z] == chosenstring)
		{
			if (Event == 0)
			{
				if (RandSwitch <= SwitchRate)
	...
	{
		if (Lattice[x][(y - 1 + 2 * L) % (L)][z] == chosenstring)
		{
			if (Event == 0)
			{
				if (RandSwitch <= SwitchRate)
	...
	{
		if (Lattice[x][y][(z + 1) % L] == chosenstring)
		{
			if (Event == 0)
			{
				if (RandSwitch <= SwitchRate)
	...
	{
		if (Lattice[x][y][(z - 1 + 2 * L) % (L)] == chosenstring)
		{
			if (Event == 0)
			{
				if (RandSwitch <= SwitchRate)
